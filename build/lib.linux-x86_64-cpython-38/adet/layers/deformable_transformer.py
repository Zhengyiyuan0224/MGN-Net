...
class DeformableTransformer(nn.Module):
    ...
    # srcs：[1, 256, 105, 104]、[1, 256, 53, 52]、[1, 256, 27, 26]、[1, 256, 14, 13]
    # masks:[1, 105, 104]、[1, 53, 52]、[1, 27, 26]、[1, 14, 13]
    # pos[1, 256, 105, 104]、[1, 256, 53, 52]、[1, 256, 27, 26]、[1, 256, 14, 13]
    # point_embed[100, 25, 256]
    def forward(self, srcs, masks, pos_embeds, query_embed):
        ...
        memory = self.encoder(          # [1, 14560, 256]
            src_flatten,                # [1, 14560, 256]
            spatial_shapes,             # [4, 2]
            level_start_index,          # [4]
            valid_ratios,               # [1, 4, 2]   TODO
            lvl_pos_embed_flatten,      # [1, 14560, 256]
            mask_flatten                # [1, 14560]
        )

        bs, _, c = memory.shape     # [1, 14560, 256]
        # TODO
        output_memory, output_proposals = self.gen_encoder_output_proposals(memory, mask_flatten, spatial_shapes) 
        # [1, 14560, 256] [1, 14560, 8]
        
        # 通过outputMemory判断这个proposal是否为文本的得分
        enc_outputs_class = self.bezier_class_embed(output_memory)  # [1, 14560, 1]
        enc_outputs_coord_unact = self.bezier_coord_embed(output_memory) + output_proposals # [1, 14560, 8]

        # select top-k curves   100
        ...
        topk_coords_unact = topk_coords_unact.detach()  #[1, 100, 8]
        reference_points = topk_coords_unact.sigmoid()  #[bs, nq, 8]
        # 将前top-K个proposal生成N(25)个点
        reference_points = self.init_points_from_bezier_proposals(reference_points)  # bs, nq, num_points, 2 [1, 100, 25, 2]
        init_reference_out = reference_points  # [1, 100, 25, 2]

        query_embed = query_embed.unsqueeze(0).expand(bs, -1, -1, -1)   # [1, 100, 25, 256]

        hs, inter_references = self.decoder(# [6, 1, 100, 25, 256]   [6, 1, 100, 25, 2]
            query_embed,        # object query          TODO
            reference_points,   # [1, 100, 25, 2]
            memory,
            spatial_shapes,
            level_start_index,
            valid_ratios,
...
class DeformableTransformerEncoderLayer(nn.Module):
    ...
    def forward(
            ...
            src,                # [1, 14560, 256]
            pos,                # [1, 14560, 256] 
            reference_points,   # [1, 14560, 4, 2]
            spatial_shapes,     # [4, 2]
            level_start_index,  # [4]
            padding_mask=None   # [1, 14560]
    ):
        # self attention
        src2 = self.self_attn(
...
class DeformableCompositeTransformerDecoderLayer(nn.Module):
    ...
    def forward(
            self,
            tgt,                    # [1, 100, 25, 256]     object query
            query_pos,              # [1, 100, 25, 256]
            reference_points,       # [1, 100, 25, 4, 2]
            src,                    # [1, 14560, 256]
            src_spatial_shapes,     # [4, 2]
            level_start_index,      # [4]
            src_padding_mask=None   # [1, 14560]
    ):
        ## input size
        ...
        q_inter = k_inter = tgt_inter = tgt.transpose(1, 2)  # (bs, n_pts, n_q, dim)
        ...
        tgt_inter = self.norm_inter(tgt_inter).transpose(1, 2)
        ...
        # ffn
        tgt = self.forward_ffn(tgt)     # [1, 100, 25, 256]
