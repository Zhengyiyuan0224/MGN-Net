...
class DETECTION_TRANSFORMER(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        """
#         samples[0].shape  torch.Size([3, 836, 832])
        if isinstance(samples, (list, torch.Tensor)):
            samples = nested_tensor_from_tensor_list(samples)
        features, pos = self.backbone(samples)
        # Multi-scale Feature Maps
        # features[0].tensors.shape     :   [1, 512, 105, 104]
        # features[0].mask.shape        :   [1, 105, 104]
        # features[1].tensors.shape     :   [1, 1024, 53, 52]
        # features[1].mask.shape        :   [1, 53, 52]
        # features[2].tensors.shape     :   [1, 2048, 27, 26]
        # features[2].mask.shape        :   [1, 27, 26]
        # pos[0].shape                  :   [1, 256, 105, 104]
        # pos[1].shape                  :   [1, 256, 53, 52]
        # pos[2].shape                  :   [1, 256, 27, 26]

        # TODO
        srcs = []
        masks = []
        for l, feat in enumerate(features):
            src, mask = feat.decompose()    # src : [1, 512, 105, 104]      mask : [1, 105, 104]
        ...
        # TODO
        # (n_proposal x n_pts, d_model) -> (n_proposal, n_pts, d_model) [100, 25, 256]
        point_embed = self.point_embed.weight.reshape((self.num_proposals, self.num_points, self.d_model)) # not shared

        (
            hs,                         # [6, 1, 100, 25, 256]
            init_reference,             # [1, 100, 25, 2]
            inter_references,           # [6, 1, 100, 25, 2]
            enc_outputs_class,          # [1, 14560, 1]
            enc_outputs_coord_unact     # [1, 14560, 8]
        ...
        # point_embed[100, 25, 256]   相当于Ojcect query
        ...
        outputs_class = torch.stack(outputs_classes)        # [6, 1, 100, 25, 1]
        outputs_text = torch.stack(outputs_texts)           # [6, 1, 100, 25, 38]
        outputs_coord = torch.stack(outputs_coords)         # [6, 1, 100, 25, 2]
        if self.boundary_head_on:
            outputs_bd_coord = torch.stack(outputs_bd_coords)   # [6, 1, 100, 25, 4]

        out = {
            'pred_logits': outputs_class[-1],           # [1, 100, 25, 1]
            'pred_text_logits': outputs_text[-1],       # [1, 100, 25, 38]
            'pred_ctrl_points': outputs_coord[-1],      # [1, 100, 25, 2]
            'pred_bd_points': outputs_bd_coord[-1] if self.boundary_head_on else None   # [1, 100, 25, 4]
        }
