...
class TransformerPureDetector(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        """
        # batched_inputs[0]['image'].shape : [3, 836, 832]
        images = self.preprocess_image(batched_inputs)      # 图像
        if self.training:
            gt_instances = [x["instances"].to(self.device) for x in batched_inputs] # 标签
            targets = self.prepare_targets(gt_instances)
            #  targets中每一个元素是一个字典 ['labels', 'beziers', 'ctrl_points', 'texts', 'bd_points']
            # labels : [22]
            # beziers: [22, 4, 2]
            # ctrl_points: [22, 25, 2]
            # texts: [22, 25]
    ...
    def prepare_targets(self, targets):
        ...
        for targets_per_image in targets:
            ...
            new_targets.append(
                {
                    "labels": gt_classes,   # 标签 是否为文字
                    "beziers": gt_beziers,  # 贝塞尔曲线
